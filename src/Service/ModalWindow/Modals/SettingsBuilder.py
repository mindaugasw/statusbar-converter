...
class SettingsBuilder(ModalWindowBuilderInterface):
    ...
    def build(self, arguments: dict[str, Any]) -> None:
        ...
        with dpg.window(label='Window title', tag=self._PRIMARY_TAG, on_close=self._onClose):
            ...
            self._buildCollapsableSection(
                'Currency converter', self._buildCurrencyConverterSettings,
            )
            self._buildCollapsableSection(
                'Distance converter', self._buildDistanceConverterSettings,
            )
            self._buildCollapsableSection(
                'Temperature converter', self._buildTemperatureConverterSettings,
            )
            ...
            self._buildCollapsableSection(
                'Unix timestamp converter', self._buildTimestampConverterSettings,
            )
        ...
        with dpg.group(horizontal=True):
            ...
            with dpg.group():
                ...
                self._appRestartNoteEditedTag = dpg.add_text(
                    noteText, show=False, color=BuilderHelper.COLOR_TEXT_RED,
                )

                dpg.add_spacer(height=25)

    def _buildCollapsableSection(
        self, title: str, buildContentCallback: Callable[[], None],
    ) -> None:
    ...
    def _buildCurrencyConverterSettings(self) -> None:
        ...
        self._registerCheckboxControl(
            dpg.add_checkbox(label='Enabled'), ConfigId.Converter_Currency_Enabled,
        )
    ...
    def _buildTimestampConverterSettings(self) -> None:
        ...
        dpg.add_spacer(height=self._SPACER_SECTION_INNER_HEIGHT)
        self._registerCheckboxControl(
            dpg.add_checkbox(label='Enabled'), ConfigId.Converter_Timestamp_Enabled,
        )
        dpg.add_spacer(height=self._SPACER_SECTION_INNER_HEIGHT)
    ...
    def _registerTextControl(
        self, tag: DpgTag, configId: ConfigParameter, castToType: Type,
    ) -> None:
    ...
    def _registerRadioControl(
        self, tag: DpgTag, configId: ConfigParameter, radioValues: SettingsRadioValues,
    ) -> None:
    ...
    def _registerDropdownControl(
        self, tag: DpgTag, configId: ConfigParameter, valueToNameMap: dict[str, str],
    ) -> None:
        dpg.set_item_callback(tag, self._controlCallback)
